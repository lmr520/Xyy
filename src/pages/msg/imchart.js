...
import { Card, ListItem, Badge, Avatar, withBadge, SocialIcon, Header, SearchBar, Divider, Overlay, Input } from 'react-native-elements';
...
export default class imchart extends Component {
  ...
  getPhotos = () => {
    ...
    // let saveImageUrl=  "https://timgsa.baidu.com/timg?image&quality=80&size=b9999_10000&sec=1534926548887&di=f107f4f8bd50fada6c5770ef27535277&imgtype=0&src=http%3A%2F%2Fpic.58pic.com%2F58pic%2F11%2F67%2F23%2F69i58PICP37.jpg";//1
    ...
    var promiseget = CameraRoll.getPhotos(fetchParams);
    promiseget.then(function (data) {
      var edges = data.edges;
      var photos = [];
      for (var index in edges) {
        // alert(edges[index].node.image.uri)
  ...
  getHistoryMessage() {
    ...
    promiseget.then(function (data) {
      ...
      for (let index = 0; index < photos.length; index++) {
        ...
        message.fromUser.avatarPath = img//1
        if (index == 0) {
          message.fromUser.displayName = "小莫"
          message.isOutgoing = true
        } else if (index == 1) {
          message.fromUser.displayName = "甘肃老湿"
          message.isOutgoing = false
        }
        else if (index == 2) {
          message.fromUser.displayName = "老宝"
          message.isOutgoing = false
        }
        else if (index == 3) {
          message.fromUser.displayName = "台州小老弟"
          message.isOutgoing = false
        }
    ...
    // alert('ExternalDirectoryPath=' + RNFS.DocumentDirectoryPath);
  ...
  onSendText = (text) => {
    ...
    message.msgType = 'text'
    message.text = text
    message.fromUser.avatarPath = this.state.photos[0]
    AuroraIController.appendMessages([message])
    this.reseveMessage();
  }
  reseveMessage = (text) => {
    ...
    message.isOutgoing = false;
    message.fromUser.displayName = "甘肃老湿"
    message.fromUser.avatarPath = this.state.photos[1]
    AuroraIController.appendMessages([message])
  }
  onTakePicture = (media) => {
    ...
    return (
      <View style={ styles.container }>
        ...
        <MessageListView style={ this.state.messageListLayout }
          ...
          receiveBubblePressedColor={ '#dddddd' }
